// OvernightOrderBottomWindow.jsx
...
export default function OvernightOrderBottomWindow({ selectedOrder, onClose, sheetData }) {
  ...
  const computedQuantity = Number(parsedNewLot) * Number(lotSize || 1);

  // computed avg (weighted): existing avg + new lots at currentPrice
  ...
  const handleAction = async (intendedAction) => {
    ...
    try {
      // validation: new lots cannot be less than current DB lots when adjusting
      if (intendedAction === 'Adjust') {
        if (parsedNewLot < currentLots) {
          setFeedback({ type: 'error', message: `Invalid lots: cannot set lots (${parsedNewLot}) less than current lots (${currentLots}).` });
          setSubmitting(false);
          return;
        }
        if (parsedNewLot <= 0) {
      ...
      if (intendedAction === 'Adjust') {
        // send computedAvg as price (2dp), computedQuantity as quantity, keep order_status null (same as your previous)
        payload = {
          ...basePayload,
        ...
        // compute closed_ltp using jobbing formula (jobbin_price like 0.08 meaning 0.08%)
        const liveLtp = Number(sheetData?.ltp ?? 0);
        // prefer selectedOrder.jobbin_price if present, fallback to jobbin_price var (from selectedOrder destructure)
