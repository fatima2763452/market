...
async function processCandidates(query, label) {
  ...
  for (const cand of candidates) {
    ...
    console.log(`[cron] ${label} - ${orderDoc.order_id || orderDoc._id} ->`, result);
  }
}
...
cron.schedule('0 14 15 * * 1-5', async () => {
  try {
    ...
    console.log(`[cron] Running intraday open-candidates ${new Date().toLocaleString('en-IN')}`);
    await processCandidates({ order_category: 'INTRADAY', order_status: 'OPEN' }, 'OPEN_INTRADAY');
  } catch (err) {
...
cron.schedule('0 0 0 * * 1-5', async () => {
  try {
    ...
    console.log(`[cron] Running midnight HOLD candidates ${new Date().toLocaleString('en-IN')}`);
    await processCandidates({ order_category: 'INTRADAY', order_status: 'HOLD' }, 'HOLD_INTRADAY');
  } catch (err) {
    console.error(err);
  }
}, { timezone: 'Asia/Kolkata' });
...
cron.schedule('0 0 0 * * 1-5', async () => {
  try {
    if (!isTradingDay(new Date())) return;
    console.log(`[cron] Running midnight OVERNIGHT candidates ${new Date().toLocaleString('en-IN')}`);
